    ...
    xhr.onload = () => {
        if (xhr.status == 200) {
            ...
            if (data.result == "success") {
                ...
                document.register_form.id_chk.value = "1";

            } else if (data.result == "fail") {
                alert("이미 사용중인 아이디입니다.");
                document.register_form.id_chk.value = "0";
                f_id.value = "";
                f_id.focus();
    ...
    xhr.onload = () => {
        if (xhr.status == 200) {
            ...
            if (data.result == "success") {
                ...
                document.register_form.email_chk.value = "1";

            } else if (data.result == "fail") {
                alert("이미 사용중인 이메일입니다.");
                document.register_form.email_chk.value = "0";
                f_email.value = "";
                f_email.focus();
...
btn_submit.addEventListener("click", () => {
     ...
     * 3. 두 비밀번호 같은지 체크
     */
    if (f.f_password.value == 0) {
        alert("비밀번호를 입력해주세요.");
        f.f_password.focus();
        return;
    }

    if (f.f_password2.value == 0) {
